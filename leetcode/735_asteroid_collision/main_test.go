...
func TestAsteroidCollisionV1(t *testing.T) {
	var tests = []*struct {
		asteroids []int
		expected  []int
	}{
...
func TestAsteroidCollisionV2(t *testing.T) {
	var tests = []*struct {
		asteroids []int
		expected  []int
	}{
	...
	for _, test := range tests {
		assert.Equal(t, test.expected, asteroidCollisionV2(test.asteroids), test.asteroids)
	}
}
